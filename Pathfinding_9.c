...
void getDesiredPos(float t, float* desiredPos, int* flag){
	...
	//desiredPos[0] = convertToPos(startRow);
	//desiredPos[1] = convertToPos(startCol);


}

/*void getDesiredPos(float to, float* desiredPos){
	desiredPos[0] = convertToPos(startRow);
	desiredPos[1] = -convertToPos(startCol);
}

/*void getDesiredPos(float t, float* desiredPos){
	float prevRow;
	float prevCol;
	int row = pathArray.waypoints[(int)t].r;
	int col = pathArray.waypoints[(int)t].c;
	if(row == -1 && col == -1){
			isEnd = true;
		}
		if(!isEnd){
			displayTextLine(4, "X: %f", convertToPos(row));
			displayTextLine(5, "Y: %f", convertToPos(col));
			desiredPos[0] = convertToPos(row);
			desiredPos[1] = -convertToPos(col);
		}
		else{
			displayTextLine(4, "X: %f", convertToPos(goalRow));
			displayTextLine(5, "Y: %f", convertToPos(goalCol));
			desiredPo[0] = convertToPos(goalRow);
			desiredPos[1] = -convertToPos(goalCol);
		}
}*/
...
/*	int p = 0;
	float proportion;
	float waypointStepsArray[40];
	while(pathArray){
		proportion=

	}*/
	waypointSteps=(float)totalTime/(float)(pathArray.pathLength + 2);
	pathArray.waypoints[0].r = startx;
	pathArray.waypoints[0].c = starty;

	pathArray.waypoints[pathArray.pathLength].r = goalx;
	pathArray.waypoints[pathArray.pathLength].c = goaly;

	pathArray.pathLength = pathArray.pathLength +1;
	//waypointSteps=10;
  /* Reset encoders and turn on PID control */
  waypointSteps=(float)totalTime/(float)(pathArray.pathLength);
  ...
  startTask(dead_reckoning);
	...
	while(nPgmTime <= (totalTime*1000))
	{
		motor[motorRight] = mPowerR;
		motor[motorLeft] = mPowerL;
